...
- **`data.py`** — pure, Streamlit-free data functions: `load_sales_data()`, `total_sales()`, `total_orders()`, `monthly_sales_trend()`, `sales_by_category()`, `sales_by_region()`. The five aggregation functions are covered by `tests/test_data.py` against a small hand-computed fixture DataFrame — not the real CSV. `load_sales_data()` itself has no test (no fixture CSV exists); it's a thin `pd.read_csv` wrapper verified manually per the Commands section. `total_amount` is trusted as-is; it is never recomputed or validated against `quantity × unit_price`.
